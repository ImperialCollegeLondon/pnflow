...
{ 
	...
	int CAMdl(4);
	const double piy180 = acos(-1.0) / 180.0;
	double mdl2SepAng(90.0*piy180);
	...
	if(elmTyp[0]=='p')
	{
		dbls CAs(nBpPors,-1.0);
		ifstream ContAngFile(fnam);
		ensure(ContAngFile);
		std::string line;
		std::getline(ContAngFile,line);
		for(size_t i=0;i<CAs.size();++i)  { ContAngFile>>CAs[i];   CAs[i]*=piy180; }
		ensure(CAs.back()>=0.0, "Wrong pore contact angles, expected "+_s(nBpPors)+" values of [0-180] in file "+fnam, -1);
		...
		for(size_t i = nBpPors; i < elemans.size(); ++i)
		{
			...
			if (trot)
				trot->setContactAngle(0.5*(CAs[elemans[i]->connection(0)->index()]+CAs[elemans[i]->connection(1)->index()]), CAMdl, mdl2SepAng);
		}
	}
	else if(elmTyp[0]=='a')
	{
		vector<double> CAs(elemans.size(),-1.0);
		ifstream ContAngFile(fnam);
		ensure(ContAngFile);
		std::string line;
		std::getline(ContAngFile,line);
		for(size_t i=0;i<CAs.size();++i) ContAngFile>>CAs[i];
		ensure(CAs.back()>=0.0, "Wrong pore+throat contact angles, expected "+_s(elemans.size())+" values of [0-180] in file "+fnam, -1);
