...
double Circle::centreEntryPrsOilInj()  { 
	...
	if(elem_.iAmAPore() && conAng > PI/2. && num_OilCentreFeederNeis != 0)  {
		...
		for(int i = 0; i < elem_.nCncts(); ++i)  {
			if(elem_.neib(i)->model()->affectsAdjEntryPc(comn_.water()))  {
		...
		ensure(iEvent == elem_.nCncts()-num_OilCentreFeederNeis);
		if(poreBodyFillAlg == "blunt2")
			entryPres = comn_.sigmaOW()*(2.*cos(conAng)/R_ - radSum);
		else
			entryPres = 2.*comn_.sigmaOW()*cos(conAng)/(R_+radSum);
	}

	double minNeiPistonEntryPrs(1e26);
	for(int i = 0; i < elem_.nCncts(); ++i)///. parallel invasion; find the easiest throat
	{
		if( elem_.neib(i)->model()->conductCOil())
			minNeiPistonEntryPrs = min( minNeiPistonEntryPrs,
				elem_.neib(i)->model()->Pc_pistonTypeRec());
	}
	//if (minNeiPistonEntryPrs<-1e35) cout<<"Rc";
	//else
	if (minNeiPistonEntryPrs<1e25) ///. Not necessary, but anyway
...
void Polygon::initOilInjection(double cappPrs)  {
	...
	double angSum(0.), normThresPress((R_*min_Pc )/comn_.sigmaOW());
	for(int i = 0; i < numCorners_; ++i)  {
		if(oilLayer_[i].exists(/*st ab le*/)) angSum += cos(cntAngAdv_ - crnHafAngs_[i]);
	}

	double rhsMaxRecConAng = (4.*shapeFactor_*angSum)
	...
	if(conAng > maxConAngSpont_ && oilLayer_[0].exists())
		...
		Pc_pistonTypeRec_ = comn_.sigmaOW()*(2.*cos(conAng)) / R_;
	else
		Pc_pistonTypeRec_ = Pc_pistonType_Drain(conAng);
...
double Polygon::centreEntryPrsOilInj()  {
	...
	if(elem_.iAmAPore() && conAng > PI/2. && num_OilCentreFeederNeis != 0)  {
		...
		string poreBodyFillAlg(comn_.poreFillAlg());
		for(int i = 0; i < elem_.nCncts(); ++i)  {
			if( elem_.neib(i)->model()->affectsAdjEntryPc( comn_.water() ) && !elem_.rockIndex() )  {
				if(poreBodyFillAlg[0] == 'o' || poreBodyFillAlg[0] == 'O')  {
					radSum += comn_.poreFillWeights(min(iEvent, 5))*
						elem_.neib(i)->model()->RRR()*
						double(rand())/double(RAND_MAX);
		...
		if(poreBodyFillAlg == "blunt2")
			pistonEntryPrs = comn_.sigmaOW()*(2.*cos(conAng)/R_ - radSum);
		else if(poreBodyFillAlg == "blunt1" || poreBodyFillAlg == "oren1")
			pistonEntryPrs = comn_.sigmaOW()*2.*cos(conAng)/(R_+radSum);
		else
			pistonEntryPrs = comn_.sigmaOW()*(1.+2.*sqrt(PI*shapeFactor_))*cos(conAng)/(R_+radSum);
	}
	else
		pistonEntryPrs = Pc_pistonTypeRec_;
...
double Polygon::Pc_pistonType_Drain(double conAng) const
{
	...
	for(int i = 0; i < numCorners_; ++i)  {
	   if(conAng < PI/2. - crnHafAngs_[i])
		  ACornsDL += cos(conAng) * cos(conAng+crnHafAngs_[i])/sin(crnHafAngs_[i]) - (PI/2. - conAng - crnHafAngs_[i]);
	}

	double funcF = (1. + sqrt(1. - 4.*shapeFactor_*ACornsDL/(cos(conAng)*cos(conAng)))) / (1. + 2.*sqrt(PI * shapeFactor_));

	return comn_.sigmaOW()*((1. + 2.*sqrt(PI*shapeFactor_)) * cos(conAng) * funcF) / R_;
}
...
bool Polygon::Pc_growStableOilLayerDrain_UseLess(double Pc, int corner)  {///. oil film growth
	...
	if (oilLayer_[corner].createOLayer(Pc,cntAngRec_,cntAngAdv_, maxConAngSpont_, crnHafAngs_[corner],  comn_.sigmaOW(),true))  {
		ensure(oilLayer_[corner].pinnedApexDist() < (R_*(1./tan(crnHafAngs_[0])+1./tan(crnHafAngs_[1]))));
