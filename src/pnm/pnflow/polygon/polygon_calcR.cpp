...
double Polygon::calcR(double pc)
{
		...
		//sumOilLayerAreas_ = 0.0;
	if(bulkFluid_ == &comn_.water() && numLayers_ == 0)
	{
		SatWater_ = 1.0;
		conductanceOil_ = 0.0;
		conductanceWater_.first = 0.0;
	...
	ensure(conductanceOil_ >= 0.0 && (conductanceWater_.first >= 0.0 || conductanceWater_.second >= 0.0));
	if ( !(conductanceOil_ >= 0.0 && (conductanceWater_.first >= 0.0 || conductanceWater_.second >= 0.0) ) )
	...
	ElectricalConductance_ += area_*(1.0-SatWater_)/comn_.oil().resistivity();
	ensure(ElectricalConductance_<1.0e64);
...
// My (Per Valvatne) very own layer conductance routine. Wohoo :-)
...
void Polygon::calcR_oilWithWaterInCorners(double cappPressure)
{
	...
	for(int i = 0; i < numCorners_; ++i)
	{
		...
		if(waterInCorner_[i].cornerExists())
		{
			...
			if(debugLevel>1 &&(apexDist < 0.0))         cout<<" apexD"<<apexDist<<" ";
	...
	SatWater_ = min(cornerAreas/area_,1.0);
	ensure( SatWater_ < 1.0 && cornerCond > 0.0);
	
	if ( debugLevel>10 && !(cornerAreas/area_ < 1.0 && cornerCond > 0.0))
	{	cout<< "============ !(cornerAreas/area_ < 1.0 && cornerCond > 0.0  =================";
		exit(-1);
	}
	
	double oilSat(max(1.0-SatWater_,0.0));
...
void Polygon::calcR_waterWithOilLayers(double cappPressure)
{
	...
	double baseLength(R_*(1.0/tan(crnHafAngs_[0])+1.0/tan(crnHafAngs_[1])));
	...
	for(int i = 0; i < numCorners_; ++i)
	{
	  ...
	  if(oilLayer_[i].LayerApex::exists(/*st ab le*/))
	  {
		...
		if(waterInCorner_[i].cornerExists())
		{
			...
			ensure(outerApexDist < (R_*(1.0/tan(crnHafAngs_[0])+1.0/tan(crnHafAngs_[1]))));
			...
			if (debugLevel>1 && ((areaWater*waterCond*(areaCornerTot - areaWater)*oilCond <=  0.0) || (oilApexBaseLengths >  baseLength*1.01 && i < 2)))
			{
				cout<<" if (debugLevel>1 && ((areaWater*waterCond*(areaCornerTot - areaWater)*oilCond <=  0.0) || (oilApexBaseLengths >  baseLength*1.01 && i < 2)))   "<< oilLayer_[i].colType_<<" ";
				exit(-1);
			}
		...
		if ( debugLevel>10 && !(layerAreaOil >= 0.0))
		{
	...
	SatWater_ = 1.0 - (cornerAreaOil + layerAreaOil)/area_;
	...
	ensure(debugLevel==0 || (watSat > 0.0 && watSat < 1.0));
...
{
	// return (3.0 * R_ * R_ * area) / (20.0 * visc);   // 1
	return (3.0 * area * area * shapeFactor_) / (5.0 * visc);      // 2
}
