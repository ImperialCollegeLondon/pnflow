...
# cnflow does not support single-pore network
#shapeToVoxel sphere  I J K  R  InV OutV(ignored in shapeToVoxel)
nErrs=0
cmds=''
	...
	for kk in range(0,3,1):
		for jj in range(0,3,1):
			for ii in range(0,3,1):
				f1.write("shapeToVoxel sphere  "+str(ii*20)+" "+str(jj*20)+" "+str(kk*20)+"     11   1\n")
...
NETWORK: F spack4;
TITLE:       spack4Net;
cycle1:    0.0      1.0E+05     0.05          T            T;
cycle2:    1.0     -1.0E+05     0.05          T            T;
cycle3:    0.0      1.0E+05     0.05          T            T;
cycle1_BC:   T     F       T     T       DP   1.0  1.0;
cycle2_BC:   T     F       T     T       DP   1.0  1.0;
cycle3_BC:   T     F       T     T       DP   1.0  1.0;
CALC_BOX:  0.0 1.0;
...
runBashScript(".", "pnextract spack4.mhd");
runBashScript(".", "pnflow    input_pnflow.dat");
runBashScript(".", "voxelImageConvert   spack4.mhd dump_spack4.raw");
if fileFloatDiffersFrom("voxelImageConvert.log","total porosity:",0.32,0.05): nErrs+=1
