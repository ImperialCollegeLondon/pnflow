...
inline void ErrorIf(bool cond, std::string message="")  {	if(cond)  {
...
#else
	#define d_assert(cond) ErrorIf(debugLevel && !(cond),  std::string("in ")+ std::string(__FUNCTION__)+ ", "\
							+std::string(__FILE__)+":"+ toStr(__LINE__) +std::string(":  ")+std::string(#cond) );
#endif

//! validation/production phase assert
#define v_assert(cond)   ErrorIf(!(cond),  std::string("in ")+ std::string(__FUNCTION__)+ ", "+std::string(__FILE__)+":"+ toStr(__LINE__)  +std::string(":  ")+std::string(#cond) );
...
inline int readInt(istringstream& in) {int n = 0; in>>n; return n;}
inline bool readBoolOr(std::string str, istringstream& in) { in>>str; return str[0] == 'T' || str[0] == 't';}

//template<typename Type>
//inline std::istream& operator>> (std::istream& in, std::vector<Type>& vec) {  for(Type &v : vec)  in >> v;  return in;  }

//template<typename Type>
//inline std::ostream& operator<< (std::ostream& ou, std::vector<Type>& vec) {  for(Type &v : vec)  ou << v<<' ';  return ou;  }
...
public:
   ...
   InputFile(const InputFile& input, const std::string& title)
	...
	InputFile(const std::string& fileName, bool skip1endl=true)
	:	verbose(false), informative(true), multiline_(skip1endl)
	{
		read(fileName);
	...
	void setTitle()
	{
		std::string prefix=getOr(std::string(""),"prefix");
		if(prefix.size())
		{

			size_t slashloc=prefix.find_first_of("\\/");
			if (slashloc<prefix.size()-1)
			{
				_folder=prefix.substr(0,slashloc+1);
				...
				#endif
					<<endl;
				prefix=prefix.substr(slashloc+1);
			}

			if (prefix.size()>1 && (*prefix.rbegin()=='/' || *prefix.rbegin()=='\\') )
			{
				_folder+=prefix;
				cout<<"Creating folder: "<<_folder<<"  "
					#if defined(_WIN32)
		...
		if (!(getVar(_name, "name") || getVar(_name, "title") || getVar(_name, "TITLE")) && prefix.empty() )
		{  _name = getOr(getOr(std::string(""),"NETWORK"),"networkFile");
			if(_name.empty()) _name = getOr(getOr(_name,"ElementDataFile"),"imageFile");
			if (_name.size()>7 && _name.substr(_name.size()-3,3)==".gz") _name = _name.substr(0,_name.size()-3);
			size_t dotloc=_name.find_last_of("."); if (dotloc<_name.size()) _name.erase(_name.find_last_of("."), std::string::npos);
			size_t slashloc=_name.find_last_of("\\/"); if (slashloc<_name.size()) _name=_name.substr(slashloc+1);
		}
		_name=prefix+_name;
	...
	void setKeyword(std::string key, std::string data)
	{
		...
		for(unsigned i = 0; i < _parsedData.size(); ++i)
			...
			{	_parsedData[i].second = data;	if(key=="prefix") setTitle();   	return;	}
		addKeyword(key,data);
		if(key=="prefix") setTitle();//prefix is controlled from match.dat file
	}
	...
	int read(std::istream& in)
	{
		...
		#else
		debugLevel=getOr(debugLevel&1,"debugLevel");
		#endif
	...
	void resetName(std::string nam) { _name=nam; }
