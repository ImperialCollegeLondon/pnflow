...
template<class T>
class piece  //! a piece of a contiguous array, used for efficient and prettier pass of array contents than using iterators
{
  protected:
	piece(): d(0), dn(0) {};
  public:
...
template<class T>
class lazyvec: public piece<T> //! a contiguous array similar to std::vallarray class, but not that ugly
{
...
inline std::istream & operator>> (std::istream & instream, std::vector< std::pair<double, double> >& tabl)
{
	for (size_t i=0; i<tabl.size();++i)
	{
		instream >> tabl[i].first >> tabl[i].second ;
	}
	if (instream.fail()) std::cout<<"Error while reading array"<<std::endl;
	return instream;
}


template<typename T> std::istream & operator>> (std::istream & instream, piece<T>& tabl)
{
	for (size_t i=0; i<tabl.size();++i)
	{
		instream >> tabl[i];
...
//#define toStr std::to_string //Bad decimal places
template<typename T> std::string toStr(const T& n){  std::ostringstream stm;  stm<<n;  return stm.str();  }

inline void      replaceInFromTo(std::string& inp, const std::string& frm, std::string const & to)
{	//return inp = std::regex_replace( inp, std::regex(from), to );
    size_t pos = 0;
    while((pos = inp.find(frm, pos)) != std::string::npos) {
		inp.replace(pos, frm.length(), to);  pos += to.length(); }
}
inline std::string replaceFromTo(std::string  inp, const std::string& frm, std::string const & to) 
{
    size_t pos = 0;
    while((pos = inp.find(frm, pos)) != std::string::npos) {
         inp.replace(pos, frm.length(), to);  pos += to.length();   }
